...
<ul class="breadcrumb">
  ...
  <li><a href="<?php echo site_url();?>index.php/user/master">Add Users from Master Data</a></li>
</ul>
 <fieldset style="">
    <legend>
      Add Users from Master Data
    </legend>
     <p class="text-center titleText">Please select a xls or xlsx file to upload new Users to the system</p>
...
<div class="col-md-4 text-center"> 
    <button  type="button" id="save2" name="save2" class="btn btn-primary formButtons">Upload and Add Users</button> 
</div>
</div>
</form>
 </fieldset> 
...
$.ajax({
       ...
       {
            $('#uploadMsg').text("Processing File and adding users...");
       ...
       processData: false,
       cache: false,
               url: '<?php echo base_url(); ?>index.php/user/processFileUser',
               data: formData2, // serializes the form's elements.
               success: function(data)
               {

                   data=JSON.parse(data);
                   if(data.error>0 && data.success==0 && data.update==0){
                      $.toast({
                            heading: 'Error',
                            text: "Update Failed. No Users added",
                   ...
                   }else if((data.success>0 || data.update>0)  && data.error==0){
                        $.toast({
                            heading: 'Success',
                            text: "All users added/updated sucessfully.",
                            position: 'bottom-center',
                            showHideTransition: 'slide',
                            icon: 'success',
                            loader: true,        // Change it to false to disable loader
                       ...
                       $.toast({
                            ...
                            text: "Users added sucessfully. Added "+data.success+", Failed (NO MOBILE AND EMAIL)"+data.error+", Updated "+data.update,
                            position: 'bottom-center',
                            showHideTransition: 'slide',
                            icon: 'warning',
                            loader: true,        // Change it to false to disable loader
